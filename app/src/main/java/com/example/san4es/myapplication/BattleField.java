...
import static com.example.san4es.myapplication.Ship.generateShipPosition;
...
public class BattleField {
     ...
     */
    private int[][] player = new int[20][20];
    int[][] enemy = new int[20][20];
    //to store matrix during generating ship's position
    int[][] savedField = new int[20][20];
    ...
    public void setPlayerField(int[][] field){

        for(int i = 0; i < N; ++i)
            for(int j = 0; j < N; ++j)
                player[i][j] = field[i][j];
    }
    public void setPlayer2Field(int[][] field){

        for(int i = 0; i < N; ++i)
            for(int j = 0; j < N; ++j)
                enemy[i][j] = field[i][j];
    }
    public void setEnemyField(int[][] field){
    ...
    public boolean addShiphand(Ship s, int[][] field) {
    ...
    private boolean checkNeighbourCells(Ship s, int[][]field) {
        ...
        int dir = s.getDirection();
            ...
            for (int i = 0; i < n; ++i) {
                ...
                //!!!
                if (p[i][0] < N && p[i][0] >= 0 && p[i][1] < N  && p[i][1] >= 0) {
                    ...
                    switch (dir) {
                        ...
                        case 0:
                            ...
                            if(field[p[i][0]][p[i][1] + 1] == 1 ||
                               ...
                               return false;}
                               else if(field[p[i][0]][p[i][1] + 1] == 1)
                                return false;


                    if(p[0][0]!=0){
                            if(field[p[0][0] - 1][p[i][1]] == 1 ||
                                    field[p[n-1][0] + 1][p[i][1]] == 1)
                                return false;}
                                else if(field[p[n-1][0] + 1][p[i][1]] == 1)
                    return false;
                        ...
                        case 1:
                            ...
                            if(field[p[i][0] + 1][p[i][1]] == 1 ||
                               ...
                               return false;}
                               else  if(field[p[i][0] + 1][p[i][1]] == 1)
                                return false;
                            if(p[0][1]!=0){
                            if(field[p[0][0]][p[0][1] - 1] == 1 ||
                                    field[p[n-1][0]][p[n-1][1] + 1] == 1)
                                return false;}
                                else  if(field[p[n-1][0]][p[n-1][1] + 1] == 1)
                                return false;
                        /*    if (i == 0) {
                                if (getBorderTouch(s) == 0) {
                                    if (field[p[i][0]][p[i][1] - 1] == 1 ||
                ...
                if (n == 1) {
                  ...
                  if (field[p[0][0] + 1][p[0][1]] == 1 ||
                        field[p[0][0] - 1][p[0][1]] == 1)
                    return false;}
                    else  if (field[p[0][0] + 1][p[0][1]] == 1 )
                        return false;
                    if(p[0][1]!=0){
                if (field[p[0][0] ][p[0][1]+1] == 1 ||
                        field[p[0][0] ][p[0][1]- 1] == 1)
                    return false;}
                    else if (field[p[0][0] ][p[0][1]+1] == 1)
                        return false;
                }
               /* else if (n == 1) {
                  //first row
    ...
    boolean hitPlayer(int ip, int jp){

        if (player[ip][jp] == SHIP) {    //here is a part of ship
            player[ip][jp] = ATTACKED_SHIP;//means attacked cell
            successfulHitPlayer = true;
            shipShooting = true;

            if(isKilledShip(ip,jp, false)){
    ...
    private void changeAttackedCell(int[][] field) {
        boolean praverkapoly=true;

        Stack<Pair> ship = new Stack<>();
        if(field==enemy)
            praverkapoly=true;
                    else praverkapoly=false;
        for(int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
        ...
        for(int i = 0; i < N; ++i) {
            for (int j = 0; j < N; ++j) {
                if(isKilledShip(i,j,praverkapoly)){
                if (field[i][j] == ATTACKED_SHIP) {
                    ...
                    if (i!=0)
                    if (field[i - 1][j] != ATTACKED_SHIP&&i!=0)
                        field[i - 1][j] = ATTACKED;
                    if (field[i][j + 1] != ATTACKED_SHIP)
                        field[i][j + 1] = ATTACKED;
                    if (j!=0)
                    if (field[i][j - 1] != ATTACKED_SHIP&&j!=0)
                        field[i][j - 1] = ATTACKED;

                    if (field[i + 1][j + 1] != ATTACKED_SHIP)
                        field[i + 1][j + 1] = ATTACKED;
                    if (i!=0&&j!=0)
                    if (field[i - 1][j - 1] != ATTACKED_SHIP&&i!=0&&j!=0)
                        field[i - 1][j - 1] = ATTACKED;
                    if (i!=0)
                    if (field[i - 1][j + 1] != ATTACKED_SHIP&&i!=0)
                        field[i - 1][j + 1] = ATTACKED;
                    if (j!=0)
                    if (field[i + 1][j - 1] != ATTACKED_SHIP&&j!=0)
                        field[i + 1][j - 1] = ATTACKED;
                }
            }
            }
    ...
    boolean isKilledShip(int ip, int jp, boolean playerAttacked){
        ...
        if(field[ip][jp] == ATTACKED_SHIP) {
          ...
          /*  if(jp!=0&&ip!=0){
            if(field[ip][jp+1] !=1  && field[ip][jp-1] != 1 && field[ip+1][jp] != 1 && field[ip-1][jp] != 1&&field[ip][jp+1] !=3  && field[ip][jp-1] != 3 && field[ip+1][jp] != 3 && field[ip-1][jp] != 3)
                return true;}
            if(jp!=0){
                if(field[ip][jp+1] !=1  && field[ip][jp-1] != 1 && field[ip+1][jp] != 1&&field[ip][jp+1] !=3  && field[ip][jp-1] != 3 && field[ip+1][jp] != 3 )
                    return true;}
            if(ip==0&&jp==0){
                if(field[ip][jp+1] !=1   && field[ip+1][jp] != 1 &&field[ip][jp+1] !=3   && field[ip+1][jp] != 3 )
                    return true;}
             if(ip!=0){
                if(field[ip][jp+1] !=1   && field[ip+1][jp] != 1 && field[ip-1][jp] != 1&&field[ip][jp+1] !=3   && field[ip+1][jp] != 3 && field[ip-1][jp] != 3)
                    return true;}
*/


          /*  if(jp==0&&ip!=0){
                if(field[ip][jp+1] == 1  || field[ip+1][jp] == 1 || field[ip-1][jp] == 1)
                    return false;}
            if(ip==0&&jp!=0) {
                if(field[ip][jp+1] == 1 || field[ip][jp-1] == 1 || field[ip+1][jp] == 1)
                    return false;
            }
            if(ip==0&&jp==0) {
                if(field[ip][jp+1] == 1  || field[ip+1][jp] == 1)
                    return false;
            }
            else if(ip!=0&&jp!=0)
            if(field[ip][jp+1] == 1 || field[ip][jp-1] == 1 || field[ip+1][jp] == 1 || field[ip-1][jp] == 1)
                return false;
*/
            ...
            if(shipDirection ==  Toward.UP ||
                ...
                while (field[ip][jp] != EMPTY&&field[ip--][jp] != ATTACKED) {
                    if(ip<0)break;// < SHIP ?????
                    if (field[ip][jp] == SHIP)
                        return false;
                    if (ip == 0)
            ...
            else if(shipDirection ==  Toward.NONE )
                return true;
        }
        return false;
    }
    ...
    Toward getAttackedNeighbourCell(int ip, int jp, int[][] field) {
        ...
        if( ip - 1 >= 0)
        if( field[ip - 1][jp] == ATTACKED_SHIP|| field[ip - 1][jp] == SHIP)
            ++up;
        if(ip + 1 < N)
        if( field[ip + 1][jp] == ATTACKED_SHIP|| field[ip + 1][jp] == SHIP)
            ++down;
        if(jp - 1 >= 0 )
        if( field[ip][jp - 1] == ATTACKED_SHIP|| field[ip][jp - 1] == SHIP)
            ++lt;
        if(jp  + 1 < N && field[ip][jp + 1] == ATTACKED_SHIP||field[ip][jp + 1] ==SHIP)
            ++rt;
        if(up > 0)
            return Toward.UP;
        if(down > 0)
